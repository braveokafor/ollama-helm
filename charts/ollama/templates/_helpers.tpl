...
{{/*
Create resources definition.
We merge the passed limits, with the GPU limits.
*/}}
{{- define "ollama.resources" -}}
{{- $existingLimits := dict }}
{{- if .Values.resources }}
  {{- $existingLimits = default dict .Values.resources.limits }}
{{- end }}
...
{{- end }}
{{- $newResources := merge (dict "limits" $newLimits) }}
{{- if .Values.resources }}
  {{- $newResources = merge .Values.resources $newResources }}
{{- end }}
{{- toYaml $newResources | nindent 2 }}
{{- end }}
